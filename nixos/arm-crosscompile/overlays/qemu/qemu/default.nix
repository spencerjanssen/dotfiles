...
let
  ...
  myglib = (glib.override { stdenv = env2; }).overrideAttrs (drv: {
    # complains about duplicate default-library settings otherwise
    mesonFlags = builtins.filter (f: f != "-Ddefault_library=static") (drv.mesonFlags or []) ++ [ "--default-library both" ];
  });
